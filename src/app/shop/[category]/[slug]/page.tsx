...
export default function ProductDetailPage({ params }: ProductPageProps) {
  ...
  // Validate product exists and belongs to the correct category route
  if (!product || product.category_id !== category) {
    notFound();
  }
  ...
  const handleAddToCart = () => {
    addItem({
      ...
      craftType: product.work_types?.[0] || "Custom Work",
    });
    setIsAdded(true);
    ...
    <main className="flex-1 bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <nav className="text-xs uppercase tracking-widest text-[#9a9a9a] font-inter">
          ...
          /{" "}
          <Link href={`/shop/${product.category_id}`} className="hover:text-[#c9a465] transition-colors">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                {product.work_types?.map((wt) => (
                  <span key={wt} className="border border-[#c9a465] text-[#c9a465] text-xs font-bold uppercase tracking-widest px-4 py-2 select-none">
                    {wt}
                  </span>
                ))}
